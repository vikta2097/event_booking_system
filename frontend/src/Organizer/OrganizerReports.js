...
const OrganizerReports = ({ currentUser }) => {
  ...
  const insights = useMemo(() => {
    ...
    if (enhancedAnalytics.revenueGrowth > 10) {
      items.push({
        ...
        message: `Your events revenue is up ${enhancedAnalytics.revenueGrowth}% compared to previous period.`
      });
    } else if (enhancedAnalytics.revenueGrowth < -10) {
      items.push({
        type: 'warning',
        title: 'Revenue Decline Detected',
        message: `Your events revenue is down ${Math.abs(enhancedAnalytics.revenueGrowth)}%.`
      });
    }
    ...
    if (enhancedAnalytics.bookingsGrowth > 15) {
      items.push({
        type: 'positive',
        title: 'Booking Surge',
        message: `Bookings for your events increased by ${enhancedAnalytics.bookingsGrowth}%.`
      });
    }
  ...
  const exportToCSV = () => {
    ...
    a.href = url;
    a.download = `my-events-reports-${new Date().toISOString().split('T')[0]}.csv`;
    a.click();
  ...
  return (
    <div className={`modern-reports-container ${darkMode ? 'dark-mode' : ''}`}>
      ...
      <div className="reports-header">
        <div>
          <h2 className="reports-title">My Events Analytics</h2>
      ...
      <div className="tabs-container">
        <div className="tabs">
          {['overview','events','trends','data'].map(tab => (
            <button
              key={tab}
        ...
        <div className="tab-content">
          ...
          {activeTab === 'events' && (
            <div className="table-card">
              <h3 className="chart-title">My Events Performance</h3>
          ...
          {activeTab === 'trends' && (
            <div className="chart-card">
              ...
              <ResponsiveContainer width="100%" height={300}>
                <AreaChart data={safeEnhancedAnalytics.dayOfWeekData}>
