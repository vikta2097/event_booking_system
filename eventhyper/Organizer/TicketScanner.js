...
import { 
  View, Text, Button, TextInput, ScrollView, StyleSheet, TouchableOpacity, FlatList 
} from "react-native";
  ...
  return (
    <ScrollView contentContainerStyle={styles.container}>
      ...
      {scannerActive && !scanResult && (
        <QRCodeScanner
          onRead={e => validateTicket(e.data)}
          reactivate={bulkScanMode}
          showMarker
          markerStyle={{ borderColor: "#4CAF50" }}
          topContent={<Text style={styles.scannerHint}>📱 Point camera at QR code</Text>}
        />
      )}
      ...
      {scanResult && (
        <View style={[styles.resultContainer, scanResult.success ? styles.success : styles.error]}>
          ...
          {!bulkScanMode && (
            <Button title="Scan Another" onPress={() => setScanResult(null)} />
          )}
        </View>
      )}
...
const styles = StyleSheet.create({
  ...
  scannerHint: { textAlign: "center", marginVertical: 10 },
